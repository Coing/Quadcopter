...
bool ms5611SelfTest(void)
{
  ...
  rawPress = ms5611GetConversion(MS5611_D1 + MS5611_OSR_4096);			//D1

  ms5611StartConversion(MS5611_D2 + MS5611_OSR_4096);
  delay_ms(CONVERSION_TIME_MS);
  rawTemp = ms5611GetConversion(MS5611_D2 + MS5611_OSR_4096);					//D2
...
float ms5611CalcTemp(int32_t deltaT)
{
  ...
  {
		/*			((1*2^-5)*2000) + (dT*tsens)*(2^(23-5))				2000 + dT*tsens * 2^23
						---------------------------------------- =  -------------------------- 为什么加上，说是为了提高精度..
											(1*2^-5)*100															   100
		*/
...
//    // Dont reader faster than we can  确保是100Hz以内
//    uint32_t now = xTaskGetTickCount();
