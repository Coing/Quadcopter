...
#define IMU_NBR_OF_BIAS_SAMPLES  40
...
void imuAccIIRLPFilter(Axis3i16* in, Axis3i16* out,
                              Axis3i32* storedValues, int32_t attenuation);
...
void arhsRead(Axis3f* gyroOut, Axis3f* accOut)
{
...
bool arhs_Calibrated(void)
{
  bool status;
	Axis3i16 acceldata,gyrodata;
	

	
	for (u8 scrap = 0; scrap < 20; scrap++)
  {
    mpu6050GetMotion6(&acceldata.x, &acceldata.y, &acceldata.z, &gyrodata.x, &gyrodata.y, &gyrodata.z);
    delay_ms(2);
  }
	...
	for(u8 i = 0; i < 40; i++)
	{
		...
		Gyro_xoffset += gyrodata.x;
		Gyro_yoffset += gyrodata.y;
		Gyro_zoffset += gyrodata.z;
		Accel_xoffset += acceldata.x;
		Accel_yoffset += acceldata.y;
		Accel_zoffset += acceldata.z;  //不知道怎么矫正，还是先别矫正了Z轴
		delay_us(200);
	}
	Gyro_xoffset=Gyro_xoffset/40;
	Gyro_yoffset=Gyro_yoffset/40;
	Gyro_zoffset=Gyro_zoffset/40;
	Accel_xoffset=Accel_xoffset/40;
	Accel_yoffset=Accel_yoffset/40;
	Accel_zoffset=Accel_zoffset/40 - 1.0/MPU6050_G_PER_LSB_4;
...
//void MPU6050_Dataanl(Axis3i16 *data_tempacc,Axis3i16 *data_tempgyr)
//{
...
void imuAccIIRLPFilter(Axis3i16* in, Axis3i16* out, Axis3i32* storedValues, int32_t attenuation)
{
  out->x = iirLPFilterSingle(in->x, attenuation, &storedValues->x);
  out->y = iirLPFilterSingle(in->y, attenuation, &storedValues->y);
  out->z = iirLPFilterSingle(in->z, attenuation, &storedValues->z);
