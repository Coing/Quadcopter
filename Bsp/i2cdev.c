...
int i2cdevInit(I2C_TypeDef *I2Cx)
{
  ...
  if (I2Cx == I2C1)
  {
	 	i2cdevResetBusI2c1(); 									//为了判断IIC总线free，试不成功
    ...
    //NVIC_InitStructure.NVIC_IRQChannelPreemptionPriority = NVIC_I2C_ER_PRI;
               ...
               uint16_t len, uint8_t *data)
{
  ...
  {
    status = I2C_Master_BufferWrite(I2Cx, &memAddress,  1, devAddress << 1, I2C_TIMEOUT);
  }
  if (status)
  {
    //TODO: Fix DMA transfer if more then 3 bytes
    status = I2C_Master_BufferRead(I2Cx, (uint8_t*)data,  len, devAddress << 1, I2C_TIMEOUT);
  }
                ...
                uint16_t len, uint8_t *data)
{
  ...
  if (memAddress != I2CDEV_NO_MEM_ADDR)
  {
    ...
    status = I2C_Master_BufferWrite(I2Cx, buffer,  len + 1, devAddress << 1, I2C_TIMEOUT);
  }
  else
  {
    status = I2C_Master_BufferWrite(I2Cx, data,  len, devAddress << 1, I2C_TIMEOUT);
  }
